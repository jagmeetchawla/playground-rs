<script lang="ts">
  ...
  // Toolchain info (read-only display)
  let clangInstalled: boolean = $state(false)
  let clangVersion: string = $state('')
  let zigInstalled: boolean = $state(false)
  let zigVersion: string = $state('')
  let swiftInstalled: boolean = $state(false)
  let swiftVersion: string = $state('')
  ...
  async function detectToolchains() {
    try {
      ...
      if (tc.zig) {
        zigInstalled = tc.zig.installed ?? false
        zigVersion = tc.zig.version ?? ''
      }
      if (tc.swiftc) {
        swiftInstalled = tc.swiftc.installed ?? false
        swiftVersion = tc.swiftc.version ?? ''
      }
    } catch (_) { /* ignore */ }
  }
  detectToolchains()
  ...
  <div class="modal-body">
    ...
    <section>
      ...
      <div class="setting-row">
        <label>Clang</label>
        {#if clangInstalled}
          <span class="toolchain-version">{clangVersion || 'installed'}</span>
        {:else}
          <span class="toolchain-version missing">not found</span>
        {/if}
      </div>

      <div class="setting-row">
        <label>Zig <span class="exp-tag">exp</span></label>
        {#if zigInstalled}
          <span class="toolchain-version">{zigVersion || 'installed'}</span>
        {:else}
          <span class="toolchain-version missing">not found</span>
        {/if}
      </div>
      ...
      <div class="setting-row">
        <label>Swift</label>
        {#if swiftInstalled}
          <span class="toolchain-version">{swiftVersion || 'installed'}</span>
        {:else}
          <span class="toolchain-version missing">not found</span>
        {/if}
      </div>
